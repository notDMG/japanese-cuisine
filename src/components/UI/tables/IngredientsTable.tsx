  ...
  if (ingredients.length === 0) {
    return (
      <div className="w-min-80 mt-5 bg-white p-8 rounded-xl shadow-2xl border text-center">
        <p className="text-mist-400 font-medium">Ingredients not found</p>
      </div>
    )
  }
    ...
    <div className="w-full mt-5">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:hidden">
        {ingredients.map((ingredient) => (
          <div 
            key={ingredient.id} 
            className="bg-white rounded-2xl p-5 shadow-xl flex flex-col justify-between"
          >
            <div className="space-y-2 text-left">
              <div className="flex justify-between items-start border-b pb-2">
                <span className="text-xs font-semibold text-gray-400 uppercase">{siteConf.tableContent.name}</span>
                <span className="text-gray-600 uppercase">{ingredient.name}</span>
              </div>
              ...
              <div className="flex justify-between items-center border-b pb-2">
                ...
                <span className="text-gray-950 font-bold">{ingredient.pricePerUnit} $</span>
              </div>
            </div>
            ...
            {isAuth && (
              <button
                ...
                disabled={isLoading}
                className="w-full mt-4 bg-red-50 text-red-600 hover:bg-red-100 font-bold py-2 px-4 rounded-xl border border-red-200 transition-colors disabled:opacity-50"
              >
                Delete
              </button>
            )}
          ...
          <tbody className="divide-y divide-gray-200 bg-white">
            {ingredients.map((ingredient) => (
              <tr key={ingredient.id} className="hover:bg-gray-50 transition-colors">
                <td className="text-gray-900 px-6 py-4 whitespace-nowrap font-medium">{ingredient.name}</td>
                ...
                <td className="text-gray-900 px-6 py-4 whitespace-nowrap font-semibold">{ingredient.pricePerUnit} ₽</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                  {isAuth && (
                    <button
                      onClick={() => handleDelete(ingredient.id)}
                      disabled={isLoading}
                      className="text-red-600 rounded-xl px-3 py-2 font-bold hover:bg-red-600 hover:text-white transition-colors disabled:opacity-50"
                    >
                      Delete
                    </button>
                  )}
